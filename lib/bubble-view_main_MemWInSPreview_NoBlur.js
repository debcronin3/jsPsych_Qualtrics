...
function generate_practice_block(
  ...
  for (var ii = 0; ii < num_trial; ii++) {
    ...
    var bubble_phase = {
      ...
      blur_image: main_img_src + "grey.jpg", // prac_img_src + prac_seq[ii] + "_30pxblur." + img_ext,
...
function generate_main_block(main_img_src, main_seq, img_ext, img_attn = []) {
  ...
  for (var ii = 0; ii < num_trial; ii++) {
    ...
    if (main_seq[ii].previewDur > 0) {
      var preview_im = {
        type: "image-keyboard-response",
        stimulus: main_img_src + main_seq[ii].scName + "." + img_ext,
        choices: jsPsych.NO_KEYS,
        trial_duration: main_seq[ii].previewDur,
      };
      block_sequence.push(preview_im);
    }
    ...
    var bubble_phase = {
      type: "bubble-view",
      org_image: main_img_src + main_seq[ii].scName + "." + img_ext,
      blur_image: main_img_src + "grey.jpg", //main_img_src + main_seq[ii].scName + "_30pxblur." + img_ext,
      ...
      data: {
        ...
        previewDur: main_seq[ii].previewDur,
      },
      on_finish: function (data) {
...
function generate_test_block(test_img_src, test_seq, img_ext) {
  ...
  for (var ii = 0; ii < num_trial; ii++) {
    var memory_phase = {
      ...
      stimulus: test_img_src + test_seq[ii].scName + "." + img_ext,
      stimulus_height: 576,
      stimulus_width: 768,
      choices: ["Yes", "No"],
      ...
      data: {
        ...
        image: test_seq[ii].scName,
        previewDur: test_seq[ii].previewDur,
        subjID: sbjId,
      },
      on_finish: function (data) {},
    };
  ...
  var toMongo = function (data) {
    fetch("https://glacial-citadel-67877.herokuapp.com/results", {
      method: "POST",
      headers: {
